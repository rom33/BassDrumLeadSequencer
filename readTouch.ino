    ...
    if (tp.z > MINPRESSURE) {
      ...
      for (slope = 0; slope < 12; slope++) {
        if (keys[slope].contains (xx, yy)) {
          if (instSelect == 0) {
            MIDI.noteOn(9, drumSet[slope], 60);
            noteTouched = slope;
            ...
            MIDI.noteOn(9 - instSelect, octave[instSelect] + slope, 60);
            return;
          }
        }
      }
      ...
      if (InstPlus.contains (xx, yy)) {
        ...
        if (instSelect > 0) {
          ...
          tft.setCursor(388, 170);
          tft.print(Sounds[Sound[instSelect]]);
          MIDI.talkMIDI(0xC9 - instSelect, Sound[instSelect], 0);
        } else {
  ...
  for (slope = 0; slope < 3; slope++) {
    ...
    if (currentPosition[slope] != lastPosition[slope]) {
      ...
      if (rotMode[slope] == 0) {
        ...
        MIDI.talkMIDI(0xB7 + slope, 0x07, Vol[slope]);
      ...
      if (rotMode[slope] == 1) {
        ...
        MIDI.talkMIDI(0xB7 + slope, 0x5b, reverb[slope]);
      ...
      if (rotMode[slope] == 2) {
        ...
        MIDI.talkMIDI(0xB7 + slope, 0x0A, pan[slope]);
